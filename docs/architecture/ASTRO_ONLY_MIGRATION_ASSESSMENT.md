...
- `src/**/*.astro`: `161`
- `src/**/*.tsx`: `94`
- `.astro` dosyalarında `client:*` hydration kullanımı: `35`
- `astro.config.mjs` içinde aktif React entegrasyonu: `@astrojs/react`
- generated inventory: `docs/reports/astro-hydration-inventory.md` (`0 low / 19 medium / 16 high`)
...
- İlk migration dalgalarında `NotificationBadge`, `QuotaUsageDisplay`, `TrendingPlaces`, `LeaderboardsDisplay`, `PricingPlans`, `UserRecommendations`, `PerformanceMonitor`, `PWAPrompt`, `TransactionHistory`, `BillingHistory`, `RewardsCatalog`, `NotificationPreferencesManager`, `NotificationCenter`, `SubscriptionManager`, `MyActivityLog`, `UserSuggestionsPanel`, `UserSearchResults` ve `HashtagExplorer` React island olmaktan çıkarıldı.
