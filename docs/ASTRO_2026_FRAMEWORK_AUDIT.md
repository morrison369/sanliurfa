...
- Anasayfa hero altı bloklar (`city-utilities`, `weather`, `stats`, `featured-places`, `historical-sites`, `food-guide`, `upcoming-events`, `latest-blog`, `cta`) `public_site_settings.homepageSections` ile admin panelden sıralı ve kapatılabilir olarak yönetilir.
...
- `/sosyal/eslesme` ürün yüzeyi canlı API tabanlı `SwipeMatchExperience` ile aktif edildi; Astro SSR + React island modeli korundu.
