...
- Backup/restore kanıtı `npm run backup:evidence` ile görünür tutulur; gerçek DB smoke `npm run smoke:db:backup-restore` ve `gate:done` içindedir.
- Runtime/Redis üretim kontrolü `npm run runtime:prod:doctor` ile secret basmadan raporlanır.
