...
| Astro-only migration kararı ve kapsamı | `docs/architecture/ASTRO_ONLY_MIGRATION_ASSESSMENT.md`, `docs/architecture/ASTRO_ONLY_MIGRATION_BACKLOG.md`, `scripts/astro-hydration-inventory.ts`, `src/lib/astro-migration-report.ts`, `scripts/astro-high-risk-feasibility.ts`, `src/lib/astro-high-risk-report.ts`, `scripts/react-surface-audit.ts`, `src/lib/react-surface-audit.ts`, `scripts/react-surface-classification.ts`, `src/lib/react-surface-classification.ts` | `src/lib/__tests__/astro-migration-report.test.ts`, `src/lib/__tests__/astro-high-risk-report.test.ts`, `src/lib/__tests__/react-surface-audit.test.ts`, `src/lib/__tests__/react-surface-classification.test.ts`, `npm run astro:migration:inventory`, `npm run astro:migration:high-risk`, `npm run astro:react:audit`, `npm run astro:react:classify` | framework seçimi, migration backlog, hydration risk envanteri, high-risk sıralaması, React paket kaldırma blokörleri, dosya bazlı sil/tut/migrate sınıflaması, `astro.config.mjs` kararları |
