...
| Astro-only migration kararı ve kapsamı | `docs/architecture/ASTRO_ONLY_MIGRATION_ASSESSMENT.md`, `docs/architecture/ASTRO_ONLY_MIGRATION_BACKLOG.md`, `scripts/astro-hydration-inventory.ts`, `src/lib/astro-migration-report.ts`, `scripts/astro-high-risk-feasibility.ts`, `src/lib/astro-high-risk-report.ts` | `src/lib/__tests__/astro-migration-report.test.ts`, `src/lib/__tests__/astro-high-risk-report.test.ts`, `npm run astro:migration:inventory`, `npm run astro:migration:high-risk` | framework seçimi, migration backlog, hydration risk envanteri, high-risk sıralaması, `astro.config.mjs` kararları |
