...
| dist/client size | **237 MB** | ⚠️ Çok büyük (CDN cost) |
...
1. eski CLIENT_SECRET (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) → invalidate
...
Şu anda place/event submission var ama **kullanıcıya hiç email gitmiyor**.
...
### 6. CDN + Image Optimization (237 MB → ?)
Tüm /uploads/ + /images/ ~200+ MB. Şu an Apache static serve. Sorunlar:
- Her image 4 saat TTL — global CDN cache hit ratio düşük
- WebP optimization eksik (sadece bazı dosyalar)
- Sharp resize on-demand kullanılmıyor

**Action plan**:
- Cloudflare R2 free tier ($0 egress) — image-only delivery
- VEYA Cloudflare ile mevcut Apache origin (already in CDN!) → Cache Rules sıkılaştır
- Build script: tüm yeni image'leri otomatik WebP + AVIF üret
...
### 10. ReviewManager Mock Data Replacement
`src/components/vendor/ReviewManager.tsx` halen mock data kullanıyor. Yeni `/vendor/yorumlar` zaten gerçek API'ye bağlı — bu component'i ya:
- A) Real API'ye geçir (place-specific yorum yönetimi /isletme/panel için)
- B) Kaldır (legacy, /vendor/yorumlar yerine geçti)
...
2. **Yüksek** (1 ay): Email + CDN + Lighthouse + SEO submission
