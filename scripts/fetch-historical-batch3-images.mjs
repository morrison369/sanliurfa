...
const LOCAL_TUNNEL_PORT = 15674;
const sleep = ms => new Promise(r => setTimeout(r, ms));

const SITES = [
  { slug: 'ceylanpinar-antik-yerlesimleri', query: 'ancient ruins mound archaeological site mesopotamia flat plain' },
  { slug: 'bozova-tarihi-koprusu',          query: 'stone arch bridge ancient ottoman river historical ruins' },
  { slug: 'siverek-ulu-camii',              query: 'old mosque stone architecture islamic medieval courtyard minaret' },
  { slug: 'akcakale-hoyugu',                query: 'ancient tell mound archaeological excavation neolithic' },
  { slug: 'viransehir-tarihi-merkezi',      query: 'ancient ruins byzantine column historic city center remains' },
];
...
async function main() {
  ...
  console.log('\n🏛️  Tarihi Yer Batch 3 Görselleri (Pexels)...\n');
  ...
  const tmpDir = path.join(projectRoot, 'dist', '_hist3_img_tmp');
  ...
  for (const s of SITES) {
    ...
    process.stdout.write(`  → ${s.slug.padEnd(36)} `);
    try {
