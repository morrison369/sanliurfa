#!/usr/bin/env node
/**
 * 12 yeni Şanlıurfa tarifi ekler (batch 3)
 * Ciğer Kebabı, Düğün Çorbası gibi eksik ikonik tarifler
 */
...
const RECIPES = [
  { name: 'Ciğer Kebabı', slug: 'ciger-kebabi', diff: 'Orta', spicy: true, veg: false, prep: 20, cook: 15, servings: 4 },
  { name: 'Düğün Çorbası', slug: 'dugun-corbasi', diff: 'Orta', spicy: false, veg: false, prep: 30, cook: 90, servings: 6 },
  { name: 'Fıstıklı Kebap', slug: 'fistikli-kebap', diff: 'Orta', spicy: false, veg: false, prep: 25, cook: 20, servings: 4 },
  { name: 'Kuru Fasulye (Urfa Usulü)', slug: 'kuru-fasulye-urfa-usulu', diff: 'Kolay', spicy: false, veg: false, prep: 15, cook: 90, servings: 6 },
  { name: 'Kanat Kebabı', slug: 'kanat-kebabi', diff: 'Kolay', spicy: true, veg: false, prep: 15, cook: 25, servings: 4 },
  { name: 'Ekmek Kadayıfı', slug: 'ekmek-kadayifi', diff: 'Orta', spicy: false, veg: true, prep: 20, cook: 30, servings: 6 },
  { name: 'Menemen (Urfa Usulü)', slug: 'menemen-urfa-usulu', diff: 'Kolay', spicy: true, veg: true, prep: 10, cook: 15, servings: 2 },
  { name: 'Harira Çorbası', slug: 'harira-corbasi', diff: 'Orta', spicy: false, veg: false, prep: 20, cook: 60, servings: 6 },
  { name: 'Topik', slug: 'topik', diff: 'Orta', spicy: false, veg: true, prep: 30, cook: 40, servings: 8 },
  { name: 'Ballı Yumurta', slug: 'balli-yumurta', diff: 'Kolay', spicy: false, veg: true, prep: 5, cook: 10, servings: 2 },
  { name: 'Patlıcan Kavurması', slug: 'patlican-kavurmasi', diff: 'Kolay', spicy: false, veg: true, prep: 15, cook: 20, servings: 4 },
  { name: 'Susamlı Çörek', slug: 'susamli-corek', diff: 'Orta', spicy: false, veg: true, prep: 60, cook: 30, servings: 12 },
];
...
Şu formatta JSON döndür (başka hiçbir şey yazma):
{
  ...
  // text[] kolonları için satır bazlı dizi — boş satırları filtrele
  ...
  try {
    await pool.query(
      ...
      [
        recipe.slug, recipe.name, data.description,
        ingredientsArr, instructionsArr, coverImage,
        recipe.prep, recipe.cook, recipe.servings,
        recipe.diff, recipe.spicy, recipe.veg,
...
async function main() {
  ...
  for (const recipe of RECIPES) {
    ...
    if (existing.rows.length > 0) {
      console.log(`[${recipe.name}] zaten mevcut, atlandı`);
      continue;
    }
