...
run('fuser -k 4321/tcp 2>/dev/null; fuser -k 4321/tcp 2>/dev/null; sleep 2; ss -tlnp | grep -E "4321|4321" || echo PORTS_FREE', 4)

print('=== 3. ENV ===', flush=True)
run('grep DATABASE_URL /home/sanliur/public_html/.env', 1)

print('=== 4. START ===', flush=True)
run('cd /home/sanliur/public_html && export PORT=4321 && export HOST=127.0.0.1 && export NODE_ENV=production && nohup node dist/server/entry.mjs > /tmp/app.log 2>&1 &', 10)

print('=== 5. HEALTH ===', flush=True)
health = run('curl -sf http://127.0.0.1:4321/api/health || echo HEALTH_FAIL', 5)

# If 4321 fails, check 4321
if 'HEALTH_FAIL' in health:
    print('Port 4321 basarisiz, 4321 deneniyor...', flush=True)
