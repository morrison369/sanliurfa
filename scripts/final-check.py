...
# Check port
print("\n=== Port 6000 Kontrolu ===", flush=True)
stdin, stdout, stderr = ssh.exec_command('ss -tulpn | grep 6000 || echo "PORT BOS"')
result = stdout.read().decode()
print(result, flush=True)

# Try curl
print("\n=== Curl Health Check ===", flush=True)
stdin, stdout, stderr = ssh.exec_command('curl -v http://127.0.0.1:6000/api/health 2>&1')
result = stdout.read().decode()
