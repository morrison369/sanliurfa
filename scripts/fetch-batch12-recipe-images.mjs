...
const LOCAL_TUNNEL_PORT = 15706;
const sleep = ms => new Promise(r => setTimeout(r, ms));

const RECIPES = [
  { slug: 'bici-bici',            query: 'turkish cold starch jelly dessert sweet summer ice rose' },
  { slug: 'zerde',                query: 'saffron rice pudding turkish sweet dessert golden yellow' },
  { slug: 'yarpuz-corbasi',       query: 'herb soup turkish lamb broth bowl traditional warm' },
  { slug: 'siron',                query: 'wheat grain lamb stew turkish traditional soup bowl winter' },
  { slug: 'urfa-dut-tatlisi',     query: 'mulberry compote fruit dessert turkish summer dark berries' },
  { slug: 'urfa-sogan-kebabi',    query: 'grilled kofta onion kebab turkish plate charcoal flatbread' },
  { slug: 'goguc-corbasi',        query: 'lamb bone broth soup turkish bowl traditional bulgur winter' },
  { slug: 'urfa-bahcivan-kebabi', query: 'baked vegetable meat casserole turkish pepper tomato eggplant' },
  { slug: 'urfa-sac-tava',        query: 'sizzling iron pan lamb vegetables turkish spicy wok stir fry' },
  { slug: 'pirasa-salatasi',      query: 'leek salad olive oil lemon turkish mezze green fresh plate' },
];
...
async function main() {
  ...
  console.log('\n🍽️  Tarif Batch 12 Görselleri (Pexels)...\n');
  ...
  const tmpDir = path.join(projectRoot, 'dist', '_batch12_recipe_img_tmp');
  ...
  for (const r of RECIPES) {
    ...
    process.stdout.write(`  → ${r.slug.padEnd(36)} `);
    try {
