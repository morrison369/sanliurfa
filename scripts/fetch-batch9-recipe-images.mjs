...
const LOCAL_TUNNEL_PORT = 15664;
const sleep = ms => new Promise(r => setTimeout(r, ms));

const RECIPES = [
  { slug: 'tirnaklı-pide',        query: 'turkish flatbread pide baked oven traditional' },
  { slug: 'urfa-nohut-corbasi',   query: 'chickpea soup bowl warm traditional turkish' },
  { slug: 'soguk-tahin-pekmez',   query: 'tahini molasses dessert traditional plate sesame' },
  { slug: 'urfa-pazi-sarma',      query: 'stuffed chard leaves dolma rice meat turkish' },
  { slug: 'urfa-sebzeli-guvec',   query: 'clay pot stew casserole vegetables meat turkish' },
  { slug: 'haydari-urfa-usulu',   query: 'yogurt dip meze herb garlic turkish traditional' },
  { slug: 'incirli-tavuk',        query: 'chicken fig stew traditional turkish sweet savory' },
  { slug: 'urfa-mercimek-kofte',  query: 'lentil bulgur kofte vegetarian meze turkish' },
  { slug: 'harran-bazlamasi',     query: 'flatbread griddle traditional turkish bread rustic' },
  { slug: 'urfa-badem-helvasi',   query: 'almond sweet dessert traditional turkish pistachio' },
];
...
async function main() {
  ...
  console.log('\n🖼️  Batch 9 Tarif Görselleri (Pexels)...\n');
  ...
  const tmpDir = path.join(projectRoot, 'dist', '_recipe9_img_tmp');
  ...
  for (const r of RECIPES) {
    ...
    process.stdout.write(`  → ${r.slug.padEnd(28)} `);
    try {
