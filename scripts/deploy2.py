...
print("2. Kill port 6000...")
cmd("fuser -k 6000/tcp 2>/dev/null; fuser -k 4321/tcp 2>/dev/null; true")
time.sleep(2)

print("3. Check port...")
port = cmd("ss -tlnp | grep 6000 || echo FREE")
print(f"   Port: {port}")

print("4. Start app...")
cmd("cd /home/sanliur/public_html && NODE_ENV=production PORT=6000 nohup node dist/server/entry.mjs >/dev/null 2>&1 &", wait=10)

print("5. Health...")
health = cmd("curl -sf http://127.0.0.1:6000/api/health || echo FAIL", wait=2)
print(f"   {health[:200]}")
