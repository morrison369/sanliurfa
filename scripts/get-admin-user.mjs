...
async function main() {
  ...
  const admins = await db.query(`SELECT id, email, full_name, role, created_at FROM users WHERE role = 'admin' ORDER BY created_at`);
  console.log('\n👤 Admin kullanıcılar:');
  for (const u of admins.rows) {
    console.log(`  ID: ${u.id}`);
    console.log(`  Email: ${u.email}`);
    console.log(`  Ad: ${u.full_name}`);
    console.log(`  Rol: ${u.role}`);
    console.log(`  Oluşturulma: ${u.created_at}`);
    console.log('  ---');
  }
