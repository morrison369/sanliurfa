...
const LOCAL_PORT = 15642;
const sleep = ms => new Promise(r => setTimeout(r, ms));

const TARGETS = [
  { slug: 'karakus-tumulu',            query: 'ancient tumulus mound archaeological ruins turkey' },
  { slug: 'suayb-sehri',               query: 'rock carved ancient cave ruins archaeological site turkey' },
  { slug: 'nemrut-dag-ziyaret-noktasi',query: 'nemrut mountain stone statues ancient ruins sunset turkey' },
];
...
async function main() {
  ...
  const remoteDir = `${REMOTE_DIR}/public/uploads/historical-sites`;
  const tmpDir = path.join(projectRoot, 'dist', '_hist_img_tmp');
  ...
  for (const t of TARGETS) {
    ...
    try {
      const imgUrl = await fetchPexels(t.query);
      if (!imgUrl) { console.log('✗ bulunamadı'); failed++; await sleep(600); continue; }

      const buf = await downloadBinary(imgUrl);
      const localPath = path.join(tmpDir, `${t.slug}.jpg`);
      fs.writeFileSync(localPath, buf);
      await sftp.put(localPath, `${remoteDir}/${t.slug}.jpg`);
      fs.unlinkSync(localPath);

      await db.query(`UPDATE historical_sites SET cover_image = $1 WHERE slug = $2`, [`/uploads/historical-sites/${t.slug}.jpg`, t.slug]);
      console.log('✓');
      done++;
    } catch (e) {
