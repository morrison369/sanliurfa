...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!locals.user?.id) {
      recordRequest(
        "POST",
        "/api/email/send-review-response",
    ...
    const { reviewerId, placeName, responseText } = validation.data as {
      reviewerId: string;
      placeName: string;
      responseText: string;
    };
    ...
    const html = getReviewResponseEmailHTML(
      reviewer.full_name || "Kullanıcı",
      placeName,
      responseText,
    );
    ...
    logger.info("Review response email sent", {
      reviewerId,
      placeName,
    ...
    return apiResponse(
      { success: true, data: { email: reviewer.email, sent: true } },
      HttpStatus.OK,
      requestId,
    );
  } catch (error) {
