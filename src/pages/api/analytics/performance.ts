...
export const GET: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    // Optional per-URL breakdown — `?byUrl=1&limit=10`
    // Slowest pages by LCP p75 (Core Web Vital priority).
    let urlBreakdown: Array<{ url: string; samples: number; lcp_p75: number; inp_p75: number; cls_p75: number }> | undefined;
    if (url.searchParams.get('byUrl') === '1') {
      ...
      const urlResult = await query<{ url: string; samples: string | number; lcp_p75: string | number; inp_p75: string | number; cls_p75: string | number }>(
        `SELECT url,
                COUNT(*) AS samples,
      ...
      urlBreakdown = urlResult.rows.map(row => ({
        url: row.url || '(unknown)',
        samples: typeof row.samples === 'number' ? row.samples : parseInt(row.samples) || 0,
