...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!Array.isArray(placeIds) || placeIds.length === 0) {
      return apiError(ErrorCode.VALIDATION_ERROR, 'placeIds dizisi gerekli', HttpStatus.UNPROCESSABLE_ENTITY, undefined, requestId);
    }

    if (placeIds.length > 100) {
      return apiError(ErrorCode.VALIDATION_ERROR, 'Tek seferde en fazla 100 mekan eklenebilir', HttpStatus.UNPROCESSABLE_ENTITY, undefined, requestId);
    }
