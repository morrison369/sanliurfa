...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (action === 'toggle_featured') {
      // Atomic toggle (HARD RULE #47): SELECT-then-UPDATE race-prone
      const updated = await queryOne(
        `UPDATE recipes SET is_featured = NOT is_featured, updated_at = NOW() WHERE id = $1 RETURNING is_featured`,
        [id]
      );
      if (!updated) return apiError(ErrorCode.NOT_FOUND, 'Tarif bulunamadı', HttpStatus.NOT_FOUND, undefined, requestId);
      return apiResponse({ success: true, is_featured: updated.is_featured }, HttpStatus.OK, requestId);
    }
    ...
    if (action === 'toggle_status') {
      ...
      if (!updated) return apiError(ErrorCode.NOT_FOUND, 'Tarif bulunamadı', HttpStatus.NOT_FOUND, undefined, requestId);
      return apiResponse({ success: true, status: updated.status }, HttpStatus.OK, requestId);
    }
    ...
    if (action === 'upsert') {
      ...
      if (existing) {
        ...
        await deleteCachePattern('recipes:*').catch(() => null);
