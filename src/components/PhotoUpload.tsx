...
function uploadWithProgress(url: string, formData: FormData, onProgress: (pct: number) => void): Promise<{ photo: Photo }> {
...
export default function PhotoUpload({ placeId, photos, onUpload, onDelete, onUpdate }: PhotoUploadProps) {
 ...
 {error && (
 <div className="p-3 bg-[rgba(239,68,68,0.1)] border border-[rgba(239,68,68,0.25)] rounded-sm text-red-700 text-sm flex justify-between">
 <span>{error}</span>
 ...
 dragOver ? 'border-red-500 bg-[rgba(239,68,68,0.1)] scale-[1.01]' : 'border-[rgba(184,115,51,0.25)] hover:border-[rgba(184,115,51,0.3)] hover:bg-[rgba(184,115,51,0.04)]'
 ...
 className="hidden"
 />
 <div className="w-16 h-16 mx-auto mb-4 bg-[rgba(184,115,51,0.06)] rounded-full flex items-center justify-center">
 <ImagePlus className="w-8 h-8 text-[#4A3828]" />
 </div>
 <p className="text-[#7A6B58] font-medium mb-1">Fotoğraf yüklemek için tıklayın veya sürükleyin</p>
 <p className="text-[#7A6B58] text-sm">JPEG, PNG, WebP, GIF • Birden fazla seçilebilir • Max 10MB/dosya</p>
 </div>

 {/* Upload Queue */}
 {queue.length > 0 && (
 <div className="space-y-2">
 <h4 className="text-sm font-medium text-[#7A6B58]">Yükleme Kuyruğu</h4>
 {queue.map(item => (
 <div key={item.id} className="flex items-center gap-3 p-3 bg-[rgba(184,115,51,0.04)] rounded-sm">
 <img src={item.preview} alt="" width={48} height={48} className="w-12 h-12 rounded object-cover flex-shrink-0" />
 <div className="flex-1 min-w-0">
 <p className="text-sm font-medium text-[#1F1410] truncate">{item.file.name}</p>
 <p className="text-xs text-[#7A6B58]">{formatSize(item.file.size)}</p>
 {(item.status === 'uploading' || item.status === 'pending') && (
 <div className="mt-1 w-full bg-[rgba(184,115,51,0.08)] rounded-full h-1.5">
 <div
 className="bg-[rgba(239,68,68,0.1)]0 h-1.5 rounded-full transition-all duration-300"
 ...
 {photos.length > 0 && (
 <div>
 <h4 className="text-sm font-medium text-[#7A6B58] mb-3">Yüklenen Fotoğraflar ({photos.length})</h4>
 <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
 {photos.map(photo => (
 <div
 key={photo.id}
 className={`relative group rounded-sm overflow-hidden aspect-square ${photo.is_primary ? 'ring-2 ring-red-500' : ''}`}
 >
 <img
 ...
 className="absolute top-2 left-2 px-2 py-1 bg-[var(--bg-card)]/90 text-[#1F1410] text-xs rounded hover:bg-[var(--bg-card)]"
 >
 Kapak Yap
 </button>
 )}
