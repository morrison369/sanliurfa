  ...
  return (
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4 text-xs text-blue-900">
      ...
      <div className="flex items-center gap-2">
        <code className="flex-1 bg-white px-2 py-1.5 rounded border border-blue-200 font-mono text-[11px] break-all">{url}</code>
        <button
          type="button"
          onClick={() => navigator.clipboard?.writeText(url)}
          className="shrink-0 px-2 py-1.5 bg-white border border-blue-200 rounded text-[11px] hover:bg-blue-100"
          title="Panoya kopyala"
        >
          📋
        </button>
      </div>
      {helpText && <p className="mt-2 text-blue-800">{helpText}</p>}
    </div>
  );
}
...
export default function IntegrationsSettings() {
  ...
  async function saveOAuthProvider(idx: number) {
    ...
    try {
      ...
      setOauthProviders(arr =>
        arr.map((p, i) =>
          i === idx
            ? { ...p, saving: false, message_status: 'error' as Status, message: (err as Error).message }
            : p,
        ),
      );
    }
  ...
  async function testIntegration(section: Section) {
    ...
    try {
      ...
      setMessages(m => ({ ...m, [section]: (err as Error).message }));
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-sm font-semibold text-gray-900">Tüm Entegrasyonları Test Et</h2>
          <p className="text-xs text-gray-500 mt-0.5">Tüm bölümleri tek seferde tarar; sonuçlar aşağıdaki badge'lerde görünür.</p>
        </div>
        <button
          type="button"
          onClick={testAll}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-32 rounded-lg border border-gray-300 px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <p className="text-xs text-gray-400 mt-1">
      ...
      <Section title="Stripe Ödemeler" icon="💳">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              <a href="https://dashboard.stripe.com/webhooks" target="_blank" rel="noopener noreferrer" className="underline">dashboard.stripe.com/webhooks</a>'tan webhook eklerken alınır. <code className="bg-gray-100 px-1 rounded">/api/billing/webhook</code> endpoint'ini Stripe'a kaydedin.
            </p>
          </div>
      ...
      <Section title="Sosyal Giriş (Google, Facebook, Twitter)" icon="🔐">
        ...
        <div className="space-y-6">
          {oauthProviders.map((p, idx) => (
            <div key={p.provider_key} className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-semibold text-gray-900">{p.provider_name}</h3>
                  {p.is_enabled && p.client_secret_set && (
                    <span className="text-xs px-2 py-0.5 bg-green-100 text-green-700 rounded">Aktif</span>
                  )}
                  {!p.is_enabled && p.client_secret_set && (
                    <span className="text-xs px-2 py-0.5 bg-gray-200 text-gray-700 rounded">Devre dışı</span>
                  )}
                  {!p.client_secret_set && (
                    <span className="text-xs px-2 py-0.5 bg-amber-100 text-amber-700 rounded">Yapılandırılmamış</span>
                  )}
                </div>
              ...
              {p.console_url && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-3 text-xs text-blue-900">
                  ...
                  <div className="mt-2">
                    ...
                    <div className="flex items-center gap-2">
                      <code className="flex-1 bg-white px-2 py-1.5 rounded border border-blue-200 font-mono text-[11px] break-all">
                        {p.redirect_uri}
                      </code>
                      <button
                        type="button"
