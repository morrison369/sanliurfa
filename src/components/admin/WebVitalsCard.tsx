...
function ratingClass(r: Rating): string {
  switch (r) {
    case 'good':              return 'bg-green-100 text-green-900 border-green-300 dark:bg-green-900/30 dark:text-green-200';
    case 'needs-improvement': return 'bg-yellow-100 text-yellow-900 border-yellow-300 dark:bg-yellow-900/30 dark:text-yellow-200';
    case 'poor':              return 'bg-red-100 text-red-900 border-red-300 dark:bg-red-900/30 dark:text-red-200';
    default:                  return 'bg-gray-100 text-gray-700 border-gray-300 dark:bg-gray-800 dark:text-gray-400';
  }
}
...
export default function WebVitalsCard() {
  ...
  return (
    <section className="bg-white dark:bg-gray-800 rounded-xl shadow-sm ring-1 ring-gray-900/5 dark:ring-gray-700/50 p-6">
      <header className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <label className="flex items-center gap-2 text-sm">
            <span className="text-gray-700 dark:text-gray-300">Başlangıç:</span>
            <input
              type="date"
              value={fromDate}
              onChange={(e) => setFromDate(e.target.value)}
              max={toDate}
              className="px-3 py-1.5 rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-gray-900 text-sm"
            />
          </label>
          <label className="flex items-center gap-2 text-sm">
            <span className="text-gray-700 dark:text-gray-300">Bitiş:</span>
            <input
              type="date"
              value={toDate}
      ...
      {error && (
        <div className="py-4 px-4 bg-red-50 dark:bg-red-900/30 text-red-800 dark:text-red-200 rounded-lg text-sm">
          {error}
        </div>
      )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {coreMetrics.map((m) => {
                ...
                return (
                  <div
                    key={m.name}
                    className={`p-4 rounded-lg border ${ratingClass(rating)}`}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {supplementary.map((m) => {
                  ...
                  return (
                    <div
                      key={m.name}
                      className={`p-3 rounded-lg border ${ratingClass(rating)}`}
                    >
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 dark:bg-gray-900/50">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-right font-semibold text-gray-700 dark:text-gray-300">Örnek</th>
                      <th className="px-3 py-2 text-right font-semibold text-gray-700 dark:text-gray-300">LCP</th>
                      <th className="px-3 py-2 text-right font-semibold text-gray-700 dark:text-gray-300">INP</th>
                      <th className="px-3 py-2 text-right font-semibold text-gray-700 dark:text-gray-300">CLS</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                    {urlBreakdown.map((row) => {
                      ...
                      return (
                        <tr key={row.url} className="bg-white dark:bg-gray-800">
                          <td className="px-4 py-2 text-gray-900 dark:text-gray-100 max-w-md">
