...
export default function PersonalizedRecommendations() {
 ...
 className="p-3 border rounded-sm hover:bg-[rgba(59,130,246,0.1)] transition cursor-pointer"
 >
 <h3 className="font-semibold truncate">{rec.name}</h3>
 <p className="text-sm text-[#7A6B58]">{rec.category}</p>
 <div className="flex items-center gap-2 mt-2">
 <div className="flex items-center gap-0.5">
 {Array.from({ length: 5 }).map((_, i) => (
 <Star key={i} className={`w-3 h-3 ${i < Math.floor(rec.rating) ? 'fill-yellow-400 text-yellow-400' : 'text-[#4A3828]'}`} />
 ))}
 </div>
 <span className="text-xs text-[#7A6B58]">%{Math.round(rec.recommendation_score * 100)}</span>
 </div>
 </a>
 ))}
 </div>
