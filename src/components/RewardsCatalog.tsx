...
const fallbackRewards: Reward[] = [
  {
    id: "local-event-access",
    reward_name: "Erken etkinlik erişimi",
    description:
      "Şanlıurfa kültür, gastronomi ve gezi etkinliklerinde erken başvuru avantajı.",
    category: "Etkinlik",
    points_cost: 250,
    available_stock: 20,
    is_active: true,
  ...
  {
    id: "featured-profile-badge",
    reward_name: "Öne çıkan profil rozeti",
    description:
      "Topluluk katkılarınızı profilinizde daha görünür gösteren özel rozet.",
    category: "Profil",
    points_cost: 500,
    available_stock: 50,
    is_active: true,
  ...
  {
    id: "local-discovery-package",
    reward_name: "Şanlıurfa keşif paketi",
    description:
      "Göbeklitepe, Balıklıgöl ve gastronomi rotaları için özel öneri paketi.",
    category: "Keşif",
    points_cost: 750,
    available_stock: 15,
    is_active: true,
...
export function RewardsCatalog() {
  ...
  const fetchRewards = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Ödüller alınamadı");
  ...
  const handleRedeem = async (rewardId: string) => {
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.error || "Ödül kullanılamadı");
      }
      ...
      setMessage({
        type: "error",
        text: err instanceof Error ? err.message : "Ödül kullanılamadı",
      });
    } finally {
      setRedeeming(null);
    }
  ...
  const categories = Array.from(new Set(rewards.map((r) => r.category))).filter(
    Boolean,
  );
              ...
              <div key={promo.id} className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-orange-700">
                    Geçerli:{" "}
                    {new Date(promo.valid_from).toLocaleDateString("tr-TR")} -{" "}
                    {new Date(promo.valid_until).toLocaleDateString("tr-TR")}
                  </p>
                </div>
                {promo.points_discount && (
      ...
      {error ? (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
          <p className="font-medium text-amber-900">
            Ödüller veritabanından alınamadı; örnek Şanlıurfa ödül kataloğu
            gösteriliyor.
          </p>
        ...
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="mt-2 text-sm">
            Puan kazandıkça yeni Şanlıurfa ödülleri burada listelenir.
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between mb-4">
                  <span className="text-2xl font-bold text-urfa-700">
                ...
                {reward.available_stock !== null &&
                  reward.available_stock > 0 && (
                    <p className="text-xs text-gray-500 mb-3">
                      {reward.available_stock} adet kaldı
                    </p>
                  )}
                ...
                <button
                  ...
                  className={`w-full py-2 rounded-lg font-medium transition ${
                    ...
                    reward.available_stock <= 0
                      ...
                      : redeeming === reward.id
                        ? "bg-urfa-400 text-white cursor-wait"
