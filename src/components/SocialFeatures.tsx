...
export function SocialFeatures() {
 ...
 const [feedItems, setFeedItems] = useState<{ id: string; object_title?: string; activity_type: string }[]>([]);
 const [trendingTags, setTrendingTags] = useState<{ id?: string; tag_slug?: string; tag_name: string; usage_count?: number }[]>([]);
 const [followers, setFollowers] = useState<{ id: string; full_name?: string; username?: string }[]>([]);
 const [following, setFollowing] = useState<{ id: string; full_name?: string; username?: string }[]>([]);
 const [pendingRequests, setPendingRequests] = useState<{ id: string; full_name?: string; username?: string; from_user_name?: string }[]>([]);
 const [matches, setMatches] = useState<{ matchId?: string; otherUserId: string; otherUserName: string; otherUsername?: string; conversationId?: string }[]>([]);
 ...
 toast.type === 'success'
 ? 'border-[rgba(34,197,94,0.2)] bg-[rgba(34,197,94,0.06)] text-emerald-900'
 : toast.type === 'error'
 ? 'border-[rgba(239,68,68,0.25)] bg-[rgba(239,68,68,0.08)] text-rose-400'
 : 'border-[rgba(59,130,246,0.2)] bg-[rgba(59,130,246,0.1)] text-blue-300'
 }`}
 >
 {toast.text}
 </div>
 ...
 <div className="flex flex-wrap gap-2 border-b border-[rgba(184,115,51,0.14)] pb-2">
 {[
 ...
 : 'bg-[var(--bg-card)] text-[#7A6B58] hover:bg-[rgba(184,115,51,0.06)]'
 }`}
 >
 {tab.label}
 </button>
 ...
 <p className="text-sm text-[#7A6B58]">Okunmamış Mesaj</p>
 <p className="mt-1 text-2xl font-bold text-[#1F1410]">{unreadCount}</p>
 </div>
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <p className="text-sm text-[#7A6B58]">Konuşma Sayısı</p>
 <p className="mt-1 text-2xl font-bold text-[#1F1410]">{conversationCount}</p>
 </div>
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <p className="text-sm text-[#7A6B58]">Günlük Swipe</p>
 <p className="mt-1 text-2xl font-bold text-[#1F1410]">
 {capabilities?.quota ? `${capabilities.quota.usedToday}/${capabilities.quota.dailyLimit}` : '-'}
 </p>
 </div>
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <p className="text-sm text-[#7A6B58]">Kalan Swipe</p>
 <p className="mt-1 text-2xl font-bold text-[#1F1410]">
 {capabilities?.quota?.remaining ?? '-'}
 </p>
 </div>
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <p className="text-sm text-[#7A6B58]">Abonelik</p>
 <p className="mt-1 text-lg font-bold text-[#1F1410]">
 ...
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <p className="text-sm text-[#7A6B58]">Plan Sayısı</p>
 <p className="mt-1 text-2xl font-bold text-[#1F1410]">{subscriptionTiers.length}</p>
 </div>
 </div>

 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <div className="flex items-center justify-between">
 <h3 className="text-base font-semibold text-[#1F1410]">Üyelik ve Erişim Özeti</h3>
 <a href="/fiyatlandirma" className="text-sm font-medium text-amber-400 hover:text-amber-400">
 Üyelik planları
 </a>
 </div>
 ...
 <div key={tier.id || tier.name} className="rounded-sm border border-[rgba(184,115,51,0.1)] p-3">
 <p className="text-sm font-semibold text-[#1F1410]">{tier.name || 'Plan'}</p>
 ...
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <h3 className="text-base font-semibold text-[#1F1410]">Aktivite Akışı</h3>
 <div className="mt-3 space-y-2">
 {feedItems.length === 0 ? (
 <p className="text-sm text-[#7A6B58]">Aktivite bulunamadı.</p>
 ) : (
 feedItems.map((item) => (
 <div key={item.id} className="rounded-sm border border-[rgba(184,115,51,0.1)] p-3">
 <p className="text-sm font-medium text-[#1F1410]">{item.object_title || item.activity_type}</p>
 <p className="text-xs text-[#7A6B58]">{item.activity_type}</p>
 </div>
 ))
 )}
 </div>
 ...
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <h3 className="text-base font-semibold text-[#1F1410]">Trend Etiketler</h3>
 <div className="mt-3 space-y-2">
 {trendingTags.length === 0 ? (
 <p className="text-sm text-[#7A6B58]">Trend etiketi bulunamadı.</p>
 ) : (
 trendingTags.map((item) => (
 <div key={item.id || item.tag_slug} className="rounded-sm border border-[rgba(184,115,51,0.1)] p-3">
 <p className="text-sm font-medium text-[#1F1410]">#{item.tag_name}</p>
 <p className="text-xs text-[#7A6B58]">{item.usage_count || 0} kullanım</p>
 </div>
 ))
 )}
 </div>
 ...
 <div className="space-y-4">
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <h3 className="text-base font-semibold text-[#1F1410]">Kullanıcı Ara (isim / username)</h3>
 <div className="mt-3 flex flex-col gap-2 sm:flex-row">
 <input
 value={userSearchQuery}
 onChange={(e) => setUserSearchQuery(e.target.value)}
 placeholder="Örn: mehmet, urfali_ali"
 className="w-full rounded-sm border border-[rgba(184,115,51,0.25)] px-3 py-2 text-sm bg-[var(--bg-card)] text-[#1F1410]"
 />
 <button
 onClick={searchUsers}
 disabled={userSearchLoading}
 className="rounded-sm bg-urfa-600 px-4 py-2 text-sm font-medium text-white hover:bg-urfa-700 disabled:opacity-60"
 >
 ...
 <div key={u.id} className="flex items-center justify-between rounded-sm border border-[rgba(184,115,51,0.1)] p-3">
 <div>
 <p className="text-sm font-medium text-[#1F1410]">{u.full_name || 'Kullanıcı'}</p>
 <p className="text-xs text-[#7A6B58]">@{u.username || 'uye'}</p>
 </div>
 <div className="flex gap-2">
 <button
 onClick={() => followUserById('follow', u.id)}
 className="rounded bg-urfa-600 px-2 py-1 text-xs text-white"
 >
 Takip Et
 </button>
 <button
 ...
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <h3 className="text-base font-semibold text-[#1F1410]">Kullanıcı ID ile Takip İşlemi</h3>
 <div className="mt-3 flex flex-col gap-2 sm:flex-row">
 <input
 ...
 className="rounded-sm bg-urfa-600 px-4 py-2 text-sm font-medium text-white hover:bg-urfa-700"
 >
 Takip Et
 </button>
 <button
 onClick={() => followUserById('unfollow')}
 className="rounded-sm bg-[rgba(184,115,51,0.08)] px-4 py-2 text-sm font-medium text-[#1F1410] hover:bg-[rgba(184,115,51,0.12)]"
 >
 Takipten Çık
 </button>
 </div>
 ...
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <h4 className="font-semibold text-[#1F1410]">Takipçiler ({followers.length})</h4>
 <div className="mt-2 space-y-2">
 {followers.slice(0, 8).map((u) => (
 <div key={u.id} className="rounded border border-[rgba(184,115,51,0.1)] p-2">
 <p className="text-sm font-medium text-[#1F1410]">{u.full_name || 'Kullanıcı'}</p>
 <p className="text-xs text-[#7A6B58]">@{u.username || 'uye'}</p>
 </div>
 ))}
 ...
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <h4 className="font-semibold text-[#1F1410]">Takip Edilenler ({following.length})</h4>
 <div className="mt-2 space-y-2">
 ...
 className="rounded bg-[rgba(184,115,51,0.06)] px-2 py-1 text-xs text-[#1F1410] hover:bg-[rgba(184,115,51,0.08)]"
 >
 Çık
 </button>
 </div>
 ...
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <h4 className="font-semibold text-[#1F1410]">Bekleyen İstekler ({pendingRequests.length})</h4>
 <div className="mt-2 space-y-2">
 {pendingRequests.length === 0 ? (
 <p className="text-sm text-[#7A6B58]">Bekleyen istek yok.</p>
 ) : (
 pendingRequests.slice(0, 8).map((r) => (
 <div key={r.id} className="rounded border border-[rgba(184,115,51,0.1)] p-2">
 <p className="text-sm font-medium text-[#1F1410]">{r.full_name || r.from_user_name || 'İstek'}</p>
 ...
 className="rounded bg-[rgba(184,115,51,0.08)] px-2 py-1 text-xs text-[#1F1410]"
 >
 Reddet
 </button>
 </div>
 ...
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <h4 className="font-semibold text-[#1F1410]">Önerilen Üyeler ({suggestedUsers.length})</h4>
 <div className="mt-3 space-y-2">
 {suggestedUsers.length === 0 ? (
 <p className="text-sm text-[#7A6B58]">Şu anda öneri bulunmuyor.</p>
 ) : (
 suggestedUsers.slice(0, 10).map((u) => (
 <div key={u.id} className="flex items-center justify-between rounded border border-[rgba(184,115,51,0.1)] p-2">
 <div>
 <p className="text-sm font-medium text-[#1F1410]">{u.name || 'Kullanıcı'}</p>
 <p className="text-xs text-[#7A6B58]">
 ...
 <h3 className="text-base font-semibold text-[#1F1410]">Eşleşen Üyeler</h3>
 <a href="/eslesme" className="rounded-sm bg-urfa-600 px-3 py-2 text-sm font-medium text-white">
 Swipe Ekranına Git
 </a>
 </div>
 <div className="mt-3 space-y-2">
 {matches.length === 0 ? (
 <p className="text-sm text-[#7A6B58]">Henüz eşleşme bulunmuyor.</p>
 ) : (
 matches.map((m) => (
 <div key={m.matchId || m.otherUserId} className="rounded-sm border border-[rgba(184,115,51,0.1)] p-3">
 <div className="flex flex-wrap items-center justify-between gap-2">
 <div>
 <p className="text-sm font-semibold text-[#1F1410]">{m.otherUserName}</p>
 <p className="text-xs text-[#7A6B58]">@{m.otherUsername || 'uye'}</p>
 </div>
 <div className="flex gap-2">
 <button
 onClick={() => followUserById('follow', m.otherUserId)}
 className="rounded bg-[rgba(59,130,246,0.1)] px-2 py-1 text-xs text-blue-300"
 >
 Takip Et
 </button>
 <button
 ...
 <div className="space-y-4">
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <h3 className="text-base font-semibold text-[#1F1410]">Mekan Etkileşimi</h3>
 ...
 className="rounded-sm bg-urfa-600 px-3 py-2 text-sm font-medium text-white"
 >
 Beğen
 </button>
 <button
 onClick={() => likePlace('unlike')}
 className="rounded-sm bg-[rgba(184,115,51,0.08)] px-3 py-2 text-sm font-medium text-[#1F1410]"
 >
 Beğeniyi Kaldır
 </button>
 <button
 ...
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <h3 className="text-base font-semibold text-[#1F1410]">Yorum Yaz</h3>
 ...
 />
 <div className="flex gap-2">
 <label className="text-sm text-[#7A6B58]">Puan:</label>
 <select
 value={reviewRating}
 onChange={(e) => setReviewRating(Number(e.target.value))}
 className="rounded-sm border border-[rgba(184,115,51,0.25)] px-2 py-1 text-sm"
 >
 {[5, 4, 3, 2, 1].map((n) => (
 ...
 className="rounded-sm bg-emerald-600 px-3 py-2 text-sm font-medium text-white"
 >
 Yorumu Gönder
 </button>
 </div>
 ...
 <div className="rounded-sm border border-[rgba(184,115,51,0.14)] bg-[var(--bg-card)] p-4">
 <h3 className="text-base font-semibold text-[#1F1410]">Yeni Mekan Öner</h3>
 <div className="mt-3 grid gap-2">
 <input
