...
export default function RecipeComments({ recipeId, recipeName, isAuthenticated }: Props) {
  ...
  useEffect(() => {
    fetch(`/api/comments?targetType=recipe&targetId=${encodeURIComponent(recipeId)}`)
      .then(r => r.json())
  ...
  const submit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch('/api/comments', {
        ...
        body: JSON.stringify({ targetType: 'recipe', targetId: recipeId, content: content.trim() }),
      });
  ...
  return (
    <div style={{ marginTop: '2rem', borderTop: '1px solid rgba(184,115,51,.12)', paddingTop: '1.5rem' }}>
      <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.2rem', fontWeight: 700, color: '#1F1410', marginBottom: '1.25rem' }}>
      ...
      {isAuthenticated ? (
        <form onSubmit={submit} style={{ marginBottom: '1.5rem', background: '#FBF6EC', border: '1px solid rgba(192,87,31,.15)', padding: '1rem', borderRadius: '6px' }}>
          <textarea
            ...
            placeholder={`${recipeName} hakkında düşüncelerinizi paylaşın...`}
            rows={3}
            maxLength={2000}
        ...
        <a
          href={`/giris?redirect=/yemek-tarifleri/${recipeId}`}
