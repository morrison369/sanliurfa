...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  if (pathname.startsWith('/api/') && (request.method === 'POST' || request.method === 'PUT' || request.method === 'PATCH')) {
    ...
    if (contentLengthHeader) {
      ...
      const isUpload = pathname.startsWith('/api/upload') || pathname.startsWith('/api/photos/upload') || pathname.startsWith('/api/files/upload');
  ...
  const csp = [
    ...
    "img-src 'self' data: blob: https://images.pexels.com https://images.unsplash.com https://plus.unsplash.com https://lh3.googleusercontent.com https://graph.facebook.com https://www.googletagmanager.com https://www.google-analytics.com",
