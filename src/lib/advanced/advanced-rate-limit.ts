...
export class TieredLimiter {
  ...
  async isAllowed(identifier: string): Promise<RateLimitResult> {
    try {
      ...
      if (!limiter) {
        logger.warn('Unknown tier for identifier', Object.assign(new Error('Unknown tier for identifier'), { identifier, tier }));
        return { allowed: true, remaining: -1, resetAt: new Date() };
      }
...
export class EndpointLimiter {
  ...
  async isAllowed(endpoint: string, identifier: string): Promise<RateLimitResult> {
    ...
    if (!limiter) {
      logger.warn('Endpoint not registered for rate limiting', Object.assign(new Error('Endpoint not registered for rate limiting'), { endpoint }));
      return { allowed: true, remaining: -1, resetAt: new Date() };
    }
...
export async function checkRateLimit(
  ...
  if (!result.allowed) {
    logger.warn('Rate limit exceeded', Object.assign(new Error('Rate limit exceeded'), {
      userId,
      ipAddress,
