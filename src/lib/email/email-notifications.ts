...
export async function notifyNewComment(
  ...
  try {
    ...
    if (!post || !post.author_id) {
      logger.warn('Yorum bildirimi gönderülemedi: yazı bulunamadı', Object.assign(new Error('Yorum bildirimi gönderilemedi'), { postId }));
      return;
    }
    ...
    if (!author || !author.email) {
      logger.warn('Yorum bildirimi gönderülemedi: yazar emaili bulunamadı', Object.assign(new Error('Yorum bildirimi gönderilemedi'), { postId, authorId: post.author_id }));
      return;
    }
    ...
    await sendEmail({
      ...
      html: `<p>${commentAuthor} adlı kullanıcı "${post.title}" yazınıza yorum yaptı:</p><p>${commentContent}</p>`
    });
  } catch (err) {
...
export async function notifyCommentReply(
  ...
  try {
    ...
    await sendEmail({
      to: recipientEmail,
      subject: `Yanıt aldınız: ${postTitle}`,
      html: `<p>${replyAuthor} adlı kullanıcı yorumunuza yanıt verdi:</p><p>${replyContent}</p>`
    });
  } catch (err) {
