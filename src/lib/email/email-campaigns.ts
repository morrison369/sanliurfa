...
export async function getAllCampaigns(limit: number = 50): Promise<EmailCampaign[]> {
  try {
    const results = await queryMany('SELECT * FROM email_campaigns ORDER BY created_at DESC LIMIT $1', [limit]) as any[] as any[];
...
export async function querySegmentUsers(segment: SegmentType, filters?: Record<string, any>): Promise<Array<{ id: string; email: string }>> {
  try {
    ...
    const results = await queryMany(query, params) as any[] as any[];
    return results.map((r: any) => ({
      id: r.id,
      email: r.email
    }));
...
export async function sendCampaign(campaignId: number | string, testMode: boolean = false): Promise<{ sent: number; failed: number } | null> {
  try {
    ...
    if (!campaign) {
      logger.error('Campaign not found', Object.assign(new Error('Campaign not found'), { campaignId }));
      return null;
    }
    ...
    if (users.length === 0) {
      logger.warn('No users found for segment', Object.assign(new Error('No users found for segment'), { segment: campaign.segment, campaignId }));
