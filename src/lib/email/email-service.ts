...
function safeEmailUrl(value: string): string {
  try {
    ...
    return url.protocol === 'http:' || url.protocol === 'https:' ? url.toString() : PUBLIC_APP_URL;
  } catch {
...
export async function sendEmail(options: {
  ...
  try {
    const mailOptions = {
      ...
      subject,
      html,
      text: text || 'Sanliurfa.com bildirimi',
    };
...
export const emailTemplates = {
  welcome: (_name: string): { subject: string; html: string } => {
    return {
    subject: 'Sanliurfa.com\'a Hoş Geldiniz!',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #dc2626;">Hoş Geldiniz!</h1>
  ...
  passwordReset: (_name: string, resetUrl: string): { subject: string; html: string } => ({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <a href="${safeEmailUrl(resetUrl)}" style="display: inline-block; padding: 12px 24px; background: #dc2626; color: white; text-decoration: none; border-radius: 8px;">Şifremi Sıfırla</a>
  ...
  reviewApproved: (_placeName: string, reviewUrl: string): { subject: string; html: string } => ({
    subject: 'Yorumunuz Onaylandı - Sanliurfa.com',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #dc2626;">Yorumunuz Onaylandı!</h1>
        <p>Yazdığınız yorum yayınlandı.</p>
        <a href="${safeEmailUrl(reviewUrl)}" style="display: inline-block; padding: 12px 24px; background: #dc2626; color: white; text-decoration: none; border-radius: 8px;">Yorumu Görüntüle</a>
        <p style="margin-top: 24px; color: #666;">Değerlendirmeniz için teşekkür ederiz!</p>
      </div>
    `,
  }),

  placeVerified: (_placeName: string, placeUrl: string): { subject: string; html: string } => ({
    subject: 'Mekanınız Doğrulandı - Sanliurfa.com',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #dc2626;">Tebrikler!</h1>
        <p>Mekanınız doğrulandı ve yayınlandı.</p>
        <a href="${safeEmailUrl(placeUrl)}" style="display: inline-block; padding: 12px 24px; background: #dc2626; color: white; text-decoration: none; border-radius: 8px;">Mekanı Görüntüle</a>
        <p style="margin-top: 24px; color: #666;">Artık müşterileriniz sizi Sanliurfa.com'da bulabilir!</p>
      </div>
    `,
  }),

  newsletter: (_title: string, content: string, unsubscribeUrl: string): { subject: string; html: string } => ({
    subject: 'Sanliurfa.com Bülteni',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 20px; background: #f3f4f6; text-align: center; font-size: 12px; color: #666;">
          ...
          <a href="${safeEmailUrl(unsubscribeUrl)}" style="color: #dc2626;">Bülten aboneliğinden çık</a>
        </div>
      </div>
    `,
  }),
