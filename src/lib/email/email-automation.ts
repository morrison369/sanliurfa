...
export async function enrollInSequence(userId: string, triggerType: SequenceTriggerType): Promise<boolean> {
  try {
    ...
    if (!sequence) {
      logger.warn('No active sequence found', Object.assign(new Error('No active sequence found'), { triggerType }));
      return false;
    }
...
export async function seedDefaultSequences(): Promise<void> {
  try {
    ...
    for (const seq of sequences) {
      ...
      if (!sequenceResult) {
        logger.warn('Failed to create sequence', Object.assign(new Error('Failed to create sequence'), { triggerType: seq.trigger_type }));
        continue;
      }
