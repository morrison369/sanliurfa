...
import { renderErrorState } from '../shared/render-states';
...
export const NOTIFICATION_TYPES: NotificationTypeDefinition[] = [
  {
    key: 'message',
    label: 'Yeni mesajlar',
    description: 'Kullanıcılardan mesaj aldığınızda',
  },
  {
    key: 'review',
    label: 'Yorum bildirimleri',
    description: 'Değerlendirmenize yorum yazıldığında',
  },
  {
    key: 'like',
    label: 'Beğeni bildirimleri',
    description: 'İçeriğiniz beğenildiğinde',
  },
  {
    key: 'follow',
    label: 'Takip bildirimleri',
    description: 'Bir kullanıcı sizi takip ettiğinde',
  },
  {
    key: 'mention',
    label: 'Bahsedilme bildirimleri',
    description: 'Sizden bahsedildiğinde',
  },
  {
    key: 'marketing',
    label: 'Pazarlama e-postaları',
    description: 'Özel teklifler, kampanyalar ve haberler',
  },
];
...
export function extractNotificationPreferenceValue(
  payload: unknown,
): NotificationPreferenceValue {
...
export function extractNotificationPreferencesSuccessMessage(
  payload: unknown,
): string {
  return extractEnvelopeMessage(payload, 'Tercihler kaydedildi.');
}

export function extractNotificationPreferencesError(
  payload: unknown,
  fallback: string,
): string {
