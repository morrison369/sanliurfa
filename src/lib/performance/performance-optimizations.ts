...
export async function batchInsert(
  ...
  try {
    ...
    if (duration > 500) {
      logger.warn(`Slow batch insert: ${tableName}`, Object.assign(new Error(`Slow batch insert: ${tableName}`), { recordCount: records.length, duration }));
    }
...
export class OptimizedCache {
  ...
  static async invalidateUserCaches(userId: string): Promise<void> {
    ...
    await Promise.all(patterns.map(key => deleteCache(key).catch((err: any) => {
