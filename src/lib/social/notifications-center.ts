import { extractEnvelopeMessage, resolveNestedEnvelopeData } from '../shared/api-envelope';
...
export function extractNotificationsCenterData(payload: unknown): {
  ...
  const notifications = Array.isArray(data)
    ? (data as unknown as NotificationsCenterItem[])
...
function renderFilters(filter: 'all' | 'unread'): string {
  ...
  return `
    <div class="flex gap-2">
      <button type="button" data-notifications-center-filter="all" class="${base} ${filter === 'all' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}">${UI_COPY_TR.notifications.all}</button>
      <button type="button" data-notifications-center-filter="unread" class="${base} ${filter === 'unread' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}">${UI_COPY_TR.notifications.unread}</button>
    </div>
  `;
}
      ...
      <div class="flex items-start justify-between gap-4">
        ...
        <div class="flex flex-shrink-0 items-center gap-2">
          ...
          <button type="button" data-notifications-center-action="delete:${item.id}" ${busy ? 'disabled' : ''} class="text-xs text-red-600 hover:text-red-700 disabled:opacity-50">
            ${busy ? UI_COPY_TR.common.processing : UI_COPY_TR.common.remove}
          </button>
        </div>
      </div>
    </div>
...
export function renderNotificationsCenter(state: NotificationsCenterState): string {
  ...
  const list = state.notifications.length === 0
    ? renderEmptyState(
        state.filter === 'unread' ? UI_COPY_TR.notifications.emptyUnread : UI_COPY_TR.notifications.emptyAll,
    ...
    <div class="space-y-4">
      ...
      <div class="mb-6 flex items-center justify-between gap-3">
        <div class="flex items-center gap-3">
          <h2 class="text-2xl font-bold text-gray-900">${UI_COPY_TR.notifications.centerTitle}</h2>
        ...
        ${canMarkAll ? `
          <button type="button" data-notifications-center-mark-all ${state.bulkActionInProgress ? 'disabled' : ''} class="text-sm text-blue-600 hover:text-blue-700 disabled:opacity-50">
            ${state.bulkActionInProgress ? UI_COPY_TR.common.processing : 'Tüm okunmamışları okundu olarak işaretle'}
          </button>
        ` : ''}
      </div>
