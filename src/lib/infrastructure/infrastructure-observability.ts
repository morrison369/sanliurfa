...
class MetricsAggregator {
  ...
  queryMetrics(
    ...
    for (const [key, series] of this.metrics.entries()) {
      ...
      if (labels) {
        // Extract JSON part after first colon (handles colons in JSON)
        const colonIndex = key.indexOf(':');
        const jsonPart = colonIndex > -1 ? key.slice(colonIndex + 1) : '{}';
        const seriesLabels = JSON.parse(jsonPart || '{}');
  ...
  evaluateMetric(metricName: string, value: number): Array<{ ruleId: string; ruleName: string; triggered: boolean }> {
    ...
    for (const [id, rule] of this.rules.entries()) {
      ...
      if (isTrigger) {
        ...
        logger.warn('Alert triggered', Object.assign(new Error('Alert triggered'), {
          rule: rule.name,
          metric: metricName,
          value,
