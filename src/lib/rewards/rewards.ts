...
export async function getRewardsList(filters?: { category?: string; tier?: string }): Promise<any[]> {
  try {
    let query = `
      SELECT
        r.*,
    ...
    const rewards = await queryMany(query, params) as any[];
    return rewards;
  } catch (error) {
...
export async function processRewardRedemption(userId: string, rewardId: string): Promise<{ success: boolean; redemptionCode?: string; error?: string }> {
  try {
