...
const getPoolConfig = (isDev: boolean) => ({
  ...
  max: isDev ? 5 : 10,           // CWP shared hosting: max 10 (was 20)
  min: isDev ? 2 : 3,            // CWP shared hosting: min 3 (was 5)
  idleTimeoutMillis: 30000,
  connectionTimeoutMillis: 5000,
  statement_timeout: 30000,
  // Phase 5: Connection reuse optimization
  application_name: 'sanliurfa-api',
  reapIntervalMillis: 5000,       // Reap idle connections every 5s
...
function monitorPoolHealth(): void {
  setInterval(() => {
    ...
    if (utilization > 80) {
      logger.warn('Connection pool high utilization', Object.assign(new Error('Connection pool high utilization'), {
    ...
    if (waitingRequests > 5) {
      logger.error('Connection pool saturation detected', Object.assign(new Error('Connection pool saturation detected'), {
        waiting: waitingRequests,
        utilization: Math.round(utilization)
      }));
    }
  ...
  try {
    ...
    if (duration > performanceThresholds.slowQueryMs) {
      ...
      if (isVerySlow) {
        ...
        logger.warn('Very slow query detected', Object.assign(new Error('Very slow query detected'), {
          duration,
...
export async function queryStream(text: string, params?: any[], onRow?: (row: any) => Promise<void>) {
  ...
  try {
    ...
    return new Promise<number>((resolve, reject) => {
      query.on('row', async (row: Record<string, unknown>) => {
        rowCount++;
        if (onRow) {
          try {
            await onRow(row);
 ...
 */
export async function insert(table: string, data: Record<string, any>, upsert?: boolean) {
  assertTable(table);
  ...
  const sql = upsert 
    ? `INSERT INTO ${table} (${keys.join(', ')}) VALUES (${placeholders}) ON CONFLICT DO NOTHING RETURNING *`
