  ...
  async processJob(jobId: string): Promise<void> {
    ...
    try {
      ...
      if (job.retries < job.maxRetries) {
        ...
        logger.warn('Job retry scheduled', Object.assign(new Error('Job retry scheduled'), {
          jobId,
          type: job.type,
          retries: job.retries,
          delayMs: delay,
        ...
        logger.error('Job failed permanently', Object.assign(new Error('Job failed permanently'), {
          jobId,
          type: job.type,
          retries: job.retries,
    ...
    try {
      ...
      for (const campaign of scheduledCampaigns) {
        try {
          ...
          logger.warn('Failed to send scheduled campaign', Object.assign(new Error('Failed to send scheduled campaign'), { campaignId: campaign.id }));
        }
      }
