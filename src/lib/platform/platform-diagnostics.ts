...
export class SelfHealingManager {
  ...
  async triggerRemediation(checkName: string): Promise<boolean> {
    ...
    if (!remediation) {
      logger.warn('No remediation for check', Object.assign(new Error('No remediation for check'), { checkName }));
      return false;
    }
    ...
    try {
      ...
      if (success) {
        ...
        logger.warn('Remediation failed', Object.assign(new Error('Remediation failed'), { checkName, duration }));
      }
    ...
    for (const [, target] of this.targets) {
      if (target.metric === metric) {
        if (!target.comparisonFn(value, target.threshold)) {
          ...
          logger.warn('SLA breach detected', Object.assign(new Error('SLA breach detected'), { metric, value, threshold: target.threshold }));
        }
      }
    }
  }
