  ...
  async get<T>(key: string, l1Ttl?: number): Promise<T | null> {
    ...
    if (l2Hit) {
      try {
        ...
        logger.warn('Failed to parse L2 cache value', Object.assign(new Error('Failed to parse L2 cache value'), { key }));
      }
    }
...
export async function warmCache(
  ...
  try {
    ...
    logger.warn('Cache warming failed', Object.assign(new Error('Cache warming failed'), { key: config.key, error: err }));
  }
}
