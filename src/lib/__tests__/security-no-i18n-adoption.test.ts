...
const HREFLANG_SOURCE_PATTERN = /\bhreflang\b|generateHreflang/;
...
describe('Static Lock — i18n adoption yasak (HARD RULE #25, Türkçe-only)', () => {
  ...
  it('source code does not define or emit hreflang', () => {
    const files = walk(SRC_ROOT, (e) => /\.(ts|tsx|astro|mjs)$/.test(e));
    const violations: string[] = [];
    for (const file of files) {
      const source = readFileSync(file, 'utf8');
      if (HREFLANG_SOURCE_PATTERN.test(source)) {
        const rel = file.split(sep).slice(file.split(sep).indexOf('src')).join('/');
        violations.push(rel);
      }
    }
    if (violations.length > 0) {
      throw new Error(
        `${violations.length} source file hreflang izi içeriyor. Türkçe-only projede hreflang yasak:\n` +
        violations.map(v => `  - ${v}`).join('\n')
      );
    }
  });
