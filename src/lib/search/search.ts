...
export async function search(filters: SearchFilters): Promise<SearchResult> {
  ...
  try {
    ...
    const resultTypes = Object.keys(results).filter(k => {
      const key = k as keyof SearchResult;
      return Array.isArray(results[key]) && (results[key] as any[]).length > 0;
    });
    await recordSearch(filters.query, results.total, resultTypes);
...
export async function recordSearch(query: string, resultsCount: number, resultTypes: string[]): Promise<void> {
  try {
    ...
    logger.error('Arama geçmişi kaydedilirken hata', error instanceof Error ? error : new Error(String(error as string)));
  }
}
...
export async function getSearchSuggestions(partial: string, limit: number = 5): Promise<string[]> {
  try {
    ...
    return (result.rows as any[]).map((r: any) => r.query);
  } catch (error) {
    logger.error('Arama önerileri alınırken hata', error instanceof Error ? error : new Error(String(error as string)));
    return [];
  }
}
