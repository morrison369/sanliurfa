...
export async function recordAdInteraction(ad_id: string, type: 'impression' | 'click'): Promise<boolean> {
  try {
    if (type === 'impression') {
      await queryOne(`UPDATE advertisements SET impressions = impressions + 1 WHERE id = $1`, [ad_id]);
    } else {
      await queryOne(`UPDATE advertisements SET clicks = clicks + 1 WHERE id = $1`, [ad_id]);
    }
    return true;
  } catch (error) {
