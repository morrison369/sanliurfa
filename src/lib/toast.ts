...
class ToastManager {
  ...
  private withDuration(type: ToastType, message: string, duration?: number): ToastOptions {
    return {
      message,
      type,
  ...
  show(options: ToastOptions) {
    ...
    inner.appendChild(this.getIcon(type));
    const msgEl = document.createElement('p');
    msgEl.className = 'text-sm font-medium';
    msgEl.textContent = message;
    inner.appendChild(msgEl);
    const closeBtn = document.createElement('button');
    closeBtn.className = 'ml-2 text-current opacity-60 hover:opacity-100';
    closeBtn.setAttribute('aria-label', 'Kapat');
    closeBtn.appendChild(this.makeCloseSvg());
  ...
  private getIcon(type: ToastType): SVGElement {
    ...
    svg.setAttribute('class', 'w-5 h-5'); svg.setAttribute('fill', 'none');
    svg.setAttribute('stroke', 'currentColor'); svg.setAttribute('viewBox', '0 0 24 24');
    const path = document.createElementNS(ns, 'path');
    path.setAttribute('stroke-linecap', 'round'); path.setAttribute('stroke-linejoin', 'round');
    path.setAttribute('stroke-width', '2'); path.setAttribute('d', paths[type]);
    svg.appendChild(path);
    return svg;
  }

  private makeCloseSvg(): SVGElement {
    const ns = 'http://www.w3.org/2000/svg';
    const svg = document.createElementNS(ns, 'svg');
    svg.setAttribute('class', 'w-4 h-4'); svg.setAttribute('fill', 'none');
    svg.setAttribute('stroke', 'currentColor'); svg.setAttribute('viewBox', '0 0 24 24');
    const path = document.createElementNS(ns, 'path');
    path.setAttribute('stroke-linecap', 'round'); path.setAttribute('stroke-linejoin', 'round');
  ...
  success(message: string, duration?: number) {
    return this.show(this.withDuration('success', message, duration));
  }

  error(message: string, duration?: number) {
    return this.show(this.withDuration('error', message, duration));
  }

  warning(message: string, duration?: number) {
    return this.show(this.withDuration('warning', message, duration));
  }

  info(message: string, duration?: number) {
    return this.show(this.withDuration('info', message, duration));
  }
}
