...
class TraceContext {
  ...
  startTrace(operationName: string, parentContext?: TraceContext): TraceContext {
    ...
    (newContext as any).traceId = traceId;
    (newContext as any).spanId = spanId;
    (newContext as any).parentSpanId = parentSpanId;
    (newContext as any).traceparent = traceparent;
    (newContext as any).tracestate = tracestate;
    (newContext as any).timestamp = Date.now();
    return newContext;
  }
