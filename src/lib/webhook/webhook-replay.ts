...
export async function requestEventReplay(
  ...
  try {
    ...
    const rows = (eventRes as any).rows;
    ...
    const replayRows = (replayRes as any).rows;
    return {
      id: replayRows[0].id,
      webhookId: replayRows[0].webhook_id,
      eventId: replayRows[0].event_id,
      status: replayRows[0].status,
      eventType: replayRows[0].event_type,
      eventData: replayRows[0].event_data,
