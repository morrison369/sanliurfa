...
export async function enrollInSequence(
  ...
  try {
    ...
    if (existing) {
      logger.info("User already enrolled in sequence", {
        userId,
    ...
    const nextSendAt = new Date(
      Date.now() + sequence.delay_minutes * 60 * 1000,
    );
  ...
  try {
    ...
    for (const enrollment of enrollments) {
      try {
        ...
        const nextSendAt = new Date(
          Date.now() + enrollment.delay_minutes * 60 * 1000,
        );
        const isLastStep =
          enrollment.current_step >= enrollment.total_steps - 1;
...
export async function seedDefaultSequences(): Promise<void> {
  try {
    ...
    for (const seq of sequences) {
      ...
      if (!sequenceResult) {
        logger.warn("Failed to create sequence", {
 ...
 */
async function createReengagementSequenceSteps(
  sequenceId: number,
): Promise<void> {
  const steps = [
    {
