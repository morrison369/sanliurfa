...
export async function addToBlacklist(ipAddress: string, reason: string, severity: 'low' | 'medium' | 'high', createdBy: string, expiresAt?: Date): Promise<boolean> {
  try {
    ...
    logger.warn('IP blacklisted', Object.assign(new Error('IP blacklisted'), { ipAddress, reason, severity }));
    return true;
  } catch (error) {
...
export async function recordRequest(userId: string | null, ipAddress: string, endpoint: string, method: string, statusCode: number, responseMsTime: number, requestSize: number, responseSize: number): Promise<void> {
  try {
    ...
    const newMinuteCount = (minuteCount ? parseInt(minuteCount as string) : 0) + 1;
    const newHourCount = (hourCount ? parseInt(hourCount as string) : 0) + 1;
    const newDayCount = (dayCount ? parseInt(dayCount as string) : 0) + 1;
...
export async function detectDDoSAttempt(ipAddress: string, endpoint: string): Promise<boolean> {
  try {
    ...
    if (count > 100) {
      ...
      logger.warn('DDoS attempt detected', Object.assign(new Error('DDoS attempt detected'), { ipAddress, endpoint, count }));
      return true;
    }
