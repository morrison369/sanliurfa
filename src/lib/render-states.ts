...
export function renderLoadingState(message = UI_COPY_TR.common.processing, className = 'py-8 text-center text-gray-500'): string {
  return `<div class="${className}">${escapeHtml(message)}</div>`;
}

export function renderEmptyState(message: string, className = 'py-12 text-center text-gray-500'): string {
  return `<div class="${className}">${escapeHtml(message)}</div>`;
}

export function renderErrorState(message: string, title = 'İşlem hatası'): string {
  return `
