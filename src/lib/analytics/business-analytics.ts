...
export async function generateAnalyticsReport(placeId: string, startDate: string, endDate: string): Promise<any> {
  try {
    const placeAnalytics = await getPlaceBusinessAnalytics(placeId, 30);
...
export async function getDashboardOverview(placeId: string): Promise<any | null> {
  try {
    ...
    if (overview) {
      await setCache(cacheKey, JSON.stringify(overview) as any, 1800);
    }
 ...
 */
export async function getUserDashboards(userId: string): Promise<BusinessDashboard[]> {
  try {
    const results = await queryMany(
