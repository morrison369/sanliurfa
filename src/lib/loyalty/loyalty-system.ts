...
export async function getAllLoyaltyTiers(): Promise<LoyaltyTier[]> {
  ...
  try {
    ...
    const tiers = await queryMany('SELECT * FROM loyalty_tiers ORDER BY tier_level ASC') as any[];
...
export async function getPointsEarningRules(): Promise<any[]> {
  ...
  try {
    ...
    const rules = await queryMany('SELECT * FROM points_earning_rules WHERE enabled = true') as any[];
