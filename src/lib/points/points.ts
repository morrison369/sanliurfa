...
export async function awardPoints(
  ...
  try {
    ...
    if (pointsToAward === 0) {
      logger.warn('No points defined for action', Object.assign(new Error('No points defined for action'), { action }));
      return null;
    }
    ...
    logger.error('Failed to award points', error instanceof Error ? error : new Error(String(error)) as any);
    return null;
  }
}
...
async function updateUserPointsTotal(userId: string): Promise<void> {
  try {
    ...
    logger.error('Failed to update points total', error instanceof Error ? error : new Error(String(error)) as any);
  }
}
...
export async function getUserPoints(userId: string): Promise<UserPoints | null> {
  try {
    ...
    logger.error('Failed to get user points', error instanceof Error ? error : new Error(String(error)) as any);
    return null;
  }
}
...
export async function getPointsHistory(userId: string, limit: number = 50): Promise<PointsTransaction[]> {
  try {
    ...
    logger.error('Failed to get points history', error instanceof Error ? error : new Error(String(error)) as any);
    return [];
  }
}
...
export async function getRewardLevels(): Promise<RewardLevel[]> {
  try {
    ...
    logger.error('Failed to get reward levels', error instanceof Error ? error : new Error(String(error)) as any);
    return [];
  }
}
...
export async function getUserRewards(userId: string): Promise<RewardLevel[]> {
  try {
    ...
    logger.error('Failed to get user rewards', error instanceof Error ? error : new Error(String(error)) as any);
    return [];
  }
}
...
export async function getPointsLeaderboard(limit: number = 20): Promise<any[]> {
  try {
    ...
    logger.error('Failed to get points leaderboard', error instanceof Error ? error : new Error(String(error)) as any);
    return [];
  }
}
