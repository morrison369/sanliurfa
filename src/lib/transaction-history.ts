      ...
      <div class="mb-2 flex items-center justify-between">
        ...
        <div class="text-right">
          <p class="text-lg font-bold ${getTransactionColorClass(transaction.transaction_type)}">
            ${transaction.points_amount > 0 ? '+' : '-'}${Math.abs(transaction.points_amount)}
          </p>
...
export function extractTransactionHistoryData(payload: unknown): {
  ...
  const transactions = Array.isArray((resolved as { transactions?: unknown }).transactions)
    ? ((resolved as { transactions: LoyaltyTransaction[] }).transactions ?? [])
    : [];

  const paginationCandidate = (resolved as { pagination?: LoyaltyTransactionPagination }).pagination;
  const pagination =
    paginationCandidate &&
...
export function renderTransactionHistory(payload: TransactionHistoryPayload): string {
