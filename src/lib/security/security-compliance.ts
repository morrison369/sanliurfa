...
export class GDPRManager {
  ...
  requestDataDeletion(userId: string): { requestId: string; delayDays: number } {
    ...
    logger.warn('Data deletion request received', Object.assign(new Error('Data deletion request received'), { userId, requestId, delayDays }));
...
export class PenetrationTestHooks {
  ...
  startTest(testName: string): void {
    ...
    if (test) {
      test.status = 'running';
      logger.warn('Penetration test started', Object.assign(new Error('Penetration test started'), { testName }));
    }
  }
  ...
  reportResults(testName: string, vulnerabilitiesFound: number, details: string[]): void {
    ...
    if (test) {
      ...
      if (vulnerabilitiesFound > 0) {
        logger.error('Penetration test found vulnerabilities', Object.assign(new Error('Penetration test found vulnerabilities'), {
          testName,
