...
export class DeviceTrustManager {
  ...
  quarantineDevice(deviceId: string, reason: string): void {
    ...
    if (device) {
      ...
      logger.warn('Device quarantined', Object.assign(new Error('Device quarantined'), { deviceId, reason }));
    }
  }
...
export class ThreatDetectionSystem {
  ...
  detectAnomaly(userId: string, action: string): ThreatAlert | null {
    ...
    logger.warn('Anomaly detected', Object.assign(new Error('Anomaly detected'), {
      alertId: id,
      userId,
      action,
      level: alert.level
...
export class SecretManagement {
  ...
  retrieveSecret(secretId: string): string | null {
    ...
    if (secret.expiresAt && Date.now() > secret.expiresAt) {
      ...
      logger.warn('Secret expired and removed', Object.assign(new Error('Secret expired and removed'), { secretId }));
      return null;
    }
