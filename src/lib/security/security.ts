...
export async function getSecurityEvents(userId: string | null, eventType?: string, limit: number = 50): Promise<SecurityEvent[]> {
  try {
    ...
    const events = await queryMany(query, params) as any[] as any[];
    return events;
  } catch (error) {
    logger.error('Failed to get security events', error instanceof Error ? error : new Error(String(error)) as any);
    return [];
  }
}
