...
class StatefulStreamProcessor {
  ...
  addEvent(event: StreamEvent): boolean {
    if (this.queue.length >= this.maxQueueSize) {
      logger.warn('Queue full - backpressure', Object.assign(new Error('Queue full - backpressure'), { processor: this.processorId }));
      return false;
    }
  ...
  processQueue(handler: (event: StreamEvent) => void): number {
    ...
    while (this.queue.length > 0) {
      ...
      try {
        ...
        logger.error('Event processing failed', Object.assign(new Error('Event processing failed'), { processor: this.processorId, error }));
      }
    }
...
class BackpressureController {
  ...
  checkBackpressure(queueSize: number): BackpressureStatus {
    ...
    if (this.status.isBackpressured) {
      logger.warn('Backpressure detected', Object.assign(new Error('Backpressure detected'), { queueSize, threshold: this.queueSizeThreshold }));
    }
