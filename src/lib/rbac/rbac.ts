...
export async function getUserPermissions(userId: string): Promise<Set<string>> {
  try {
    ...
    const permissions = new Set((result.rows as any[]).map((r: any) => r.name).filter(Boolean));
    ...
    logger.error('Kullanici izinleri alınırken hata', error instanceof Error ? error : new Error(String(error as string)));
    return new Set();
  }
}
...
export async function getRolePermissions(roleId: string): Promise<Array<{ id: string; name: string; resource: string; action: string }>> {
  try {
    ...
    return result.rows as Array<{ id: string; name: string; resource: string; action: string }>;
  } catch (error) {
...
export async function getAllRoles(): Promise<Array<{ id: string; name: string; is_system: boolean }>> {
  try {
    ...
    return result.rows as Array<{ id: string; name: string; is_system: boolean }>;
  } catch (error) {
