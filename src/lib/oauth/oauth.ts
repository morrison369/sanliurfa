...
export async function getOAuthProvider(providerKey: string): Promise<OAuthProvider | null> {
  try {
    ...
    logger.error('Failed to get OAuth provider', error instanceof Error ? error : new Error(String(error)) as any);
    return null;
  }
}
...
export async function listOAuthProviders(): Promise<OAuthProvider[]> {
  try {
    ...
    logger.error('Failed to list OAuth providers', error instanceof Error ? error : new Error(String(error)) as any);
    return [];
  }
}
...
export async function unlinkOAuthAccount(userId: string, accountId: string): Promise<boolean> {
  try {
    ...
    if (accountCount?.count <= 1) {
      logger.warn('Cannot unlink only OAuth account', Object.assign(new Error('Cannot unlink only OAuth account'), { userId, accountId }));
      return false;
    }
