...
class CommandHandler {
  ...
  async execute(command: Command): Promise<CommandResult> {
    ...
    try {
      ...
      logger.error('Command execution failed', Object.assign(new Error('Command execution failed'), { type: command.type, error }));
      return {
        status: 'failure',
        commandId: `cmd-${Date.now()}-${++this.counter}`,
        error: String(error)
  ...
  async handle(event: { type: string; data: any }): Promise<void> {
    ...
    for (const handler of handlers) {
      try {
        ...
        logger.error('Event handler failed', Object.assign(new Error('Event handler failed'), { type: event.type, error }));
      }
    }
