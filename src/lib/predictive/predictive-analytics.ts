...
export async function recordRecommendationFeedback(recommendationId: string, clicked: boolean, rating?: number): Promise<boolean> {
  try {
    ...
    logger.error('Failed to record feedback', error instanceof Error ? error : new Error(String(error)) as any);
    return false;
  }
}
...
export async function getModelPerformanceMetrics(): Promise<any> {
  try {
    ...
    logger.error('Failed to get model metrics', error instanceof Error ? error : new Error(String(error)) as any);
    return {};
  }
}
