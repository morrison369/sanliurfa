...
function renderFilters(filter: 'all' | 'unread'): string {
  ...
  return `
    <div class="flex gap-2">
      <button type="button" data-notifications-center-filter="all" class="${base} ${filter === 'all' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}">Tümü</button>
      <button type="button" data-notifications-center-filter="unread" class="${base} ${filter === 'unread' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}">Okunmamış</button>
    </div>
  `;
}
  ...
  const list = state.notifications.length === 0
    ? `
      <div class="py-12 text-center text-gray-500">
        <p>${state.filter === 'unread' ? 'Okunmamış bildirim bulunmuyor.' : 'Gösterilecek bildirim bulunmuyor.'}</p>
      </div>
    `
      ...
      <div class="mb-6 flex items-center justify-between gap-3">
        ...
        ${canMarkAll ? `
          <button type="button" data-notifications-center-mark-all ${state.bulkActionInProgress ? 'disabled' : ''} class="text-sm text-blue-600 hover:text-blue-700 disabled:opacity-50">
            ${state.bulkActionInProgress ? 'İşleniyor...' : 'Tüm bildirimleri okundu olarak işaretle'}
          </button>
        ` : ''}
      </div>
