...
export async function emailOnSubscriptionCreated(
  ...
  try {
    ...
    if (!user || !user.email) {
      logger.warn('Cannot send subscription created email: user not found', Object.assign(new Error('Cannot send subscription created email: user not found'), { userId }));
      return false;
    }
    ...
    if (!tier) {
      logger.warn('Cannot send subscription created email: tier not found', Object.assign(new Error('Cannot send subscription created email: tier not found'), { tierId }));
      return false;
    }
...
export async function emailOnPlanUpgrade(
  ...
  try {
    ...
    if (!user || !user.email) {
      logger.warn('Cannot send upgrade email: user not found', Object.assign(new Error('Cannot send upgrade email: user not found'), { userId }));
      return false;
    }
    ...
    if (!oldTier || !newTier) {
      logger.warn('Cannot send upgrade email: tier not found', new Error('Cannot send upgrade email: tier not found'));
      return false;
    }
...
export async function emailOnPlanDowngrade(
  ...
  try {
    ...
    if (!user || !user.email) {
      logger.warn('Cannot send downgrade email: user not found', Object.assign(new Error('Cannot send downgrade email: user not found'), { userId }));
      return false;
    }
    ...
    if (!oldTier || !newTier) {
      logger.warn('Cannot send downgrade email: tier not found', new Error('Cannot send downgrade email: tier not found'));
      return false;
    }
...
export async function emailOnSubscriptionCancelled(
  ...
  try {
    ...
    if (!user || !user.email) {
      logger.warn('Cannot send cancellation email: user not found', Object.assign(new Error('Cannot send cancellation email: user not found'), { userId }));
      return false;
    }
    ...
    if (!tier) {
      logger.warn('Cannot send cancellation email: tier not found', Object.assign(new Error('Cannot send cancellation email: tier not found'), { tierId }));
      return false;
    }
...
export async function emailOnPaymentSuccess(
  ...
  try {
    ...
    if (!user || !user.email) {
      logger.warn('Cannot send payment success email: user not found', Object.assign(new Error('Cannot send payment success email: user not found'), { userId }));
      return false;
    }
    ...
    if (!tier) {
      logger.warn('Cannot send payment success email: tier not found', Object.assign(new Error('Cannot send payment success email: tier not found'), { tierId }));
      return false;
    }
...
export async function emailOnPaymentFailed(
  ...
  try {
    ...
    if (!user || !user.email) {
      logger.warn('Cannot send payment failed email: user not found', Object.assign(new Error('Cannot send payment failed email: user not found'), { userId }));
      return false;
    }
    ...
    if (!tier) {
      logger.warn('Cannot send payment failed email: tier not found', Object.assign(new Error('Cannot send payment failed email: tier not found'), { tierId }));
      return false;
    }
...
export async function emailOnSubscriptionRenewal(
  ...
  try {
    ...
    if (!user || !user.email) {
      logger.warn('Cannot send renewal email: user not found', Object.assign(new Error('Cannot send renewal email: user not found'), { userId }));
      return false;
    }
    ...
    if (!tier) {
      logger.warn('Cannot send renewal email: tier not found', Object.assign(new Error('Cannot send renewal email: tier not found'), { tierId }));
      return false;
    }
