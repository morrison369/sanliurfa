  ...
  for (let i = 0; i < html.length; i++) {
    ...
    if (inTag) {
      if (char === '>') {
        ...
        if (normalizedTag.startsWith('script')) skipUntil = 'script';
        if (normalizedTag.startsWith('style')) skipUntil = 'style';
        if (skipUntil && normalizedTag.startsWith(`/${skipUntil}`)) skipUntil = null;
        tag = '';
        inTag = false;
      } else {
        tag += char;
...
async function fetchSourceDigest(source: CityContentSource): Promise<{ ok: boolean; digest: string }> {
  ...
  try {
    ...
    const clean = extractVisibleText(text)
      .replace(/\s+/g, ' ')
      .trim()
      .slice(0, 900);
