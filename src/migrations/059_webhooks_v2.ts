...
export const migration_059_webhooks = async (pool: Pool) => {
  try {
    ...
    await pool.query(`
      ...
      ALTER TABLE webhooks ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT NOW();
    `);
    ...
    await pool.query(`
      ALTER TABLE webhook_events ADD COLUMN IF NOT EXISTS status VARCHAR(20) DEFAULT 'pending';
